...
public static class AspectCoreInjectorExtensions
{
    ...
    {
        var lifetime = d.LifetimeType.ToAspectCoreLifetime();
        services.AddDelegate(d.ServiceType, _ => d.InstanceFuncForImplementation(), lifetime);
    }
    ...
    private static void InstanceSelfFuncRegister(IServiceContext services, DependencyProxyDescriptor d)
    {
        ...
        switch (lifetime)
        {
            case Lifetime.Scoped:
                services.AddDelegate(_ => d.InstanceFuncForImplementation(), Lifetime.Scoped);
                break;

            case Lifetime.Singleton:
                services.AddDelegate(_ => d.InstanceFuncForImplementation(), Lifetime.Singleton);
                break;

            case Lifetime.Transient:
                services.AddDelegate(_ => d.InstanceFuncForImplementation());
                break;
